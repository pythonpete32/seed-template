...
//    bytes32 private constant ARAGON_FUNDRAISING_ID = 0x668ac370eed7e5861234d1c0a1e512686f53594fcb887e5bcecc35675a4becac;
    ...
    */
        ...
        public
    {
        ...
        Tollgate tollgate = _installTollgate(dao, _tollgateFeeToken, _tollgateFeeAmount, address(fundingPoolVault));
        ...
        _storeDeployedContractsTxOne(dao, acl, dandelionVoting, fundingPoolVault, hookedTokenManager, _collateralToken, issuance);
    }
    ...
    */
        ...
        public
    {
        ...
        _setupCollateralToken(dao, acl, _virtualSupply, _virtualBalance, _reserveRatio, _slippage, _tapRate, _tapFloor);
        ...
        internal
    {
        ...
        (,,,,,address collateralToken,) = _getDeployedContractsTxOne();
    ...
    internal
    {
        ( , , DandelionVoting dandelionVoting, ,, address collateralToken,) = _getDeployedContractsTxOne();
    ...
    function _createHookedTokenManagerPermissions() internal {
        ...
        (,, DandelionVoting dandelionVoting,, HookedTokenManager hookedTokenManager, , Issuance issuance) = _getDeployedContractsTxOne();
        ...
        minters[0] = address(marketMaker);
        minters[1] = address(issuance);

        _createPermissions(acl, minters, hookedTokenManager, hookedTokenManager.MINT_ROLE(), dandelionVoting);
    ...
    function _createFundraisingPermissions() internal {
        ...
        acl.createPermission(ANY_ENTITY, controller, controller.WITHDRAW_ROLE(), dandelionVoting);
    }

    // Temporary Storage functions //

    function _storeDeployedContractsTxOne(Kernel _dao, ACL _acl, DandelionVoting _dandelionVoting, Vault _agentOrVault, HookedTokenManager _hookedTokenManager, address _collateralToken, Issuance _issuance)
        internal
    {
        ...
        deployedContracts.issuance = _issuance;
    }

    function _getDeployedContractsTxOne() internal returns (Kernel, ACL, DandelionVoting, Vault, HookedTokenManager, address, Issuance) {
        DeployedContracts storage deployedContracts = senderDeployedContracts[msg.sender];
        return (
            ...
            deployedContracts.collateralToken,
            deployedContracts.issuance
        );
    }
